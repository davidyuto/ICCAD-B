    ...
    for (int i = 0; i < pin->numPorts(); i++) {
        ...
        for (int j = 0; j < port->numItems(); j++) {
            if (port->itemType(j) == lefiGeomLayerE) {
                ...
                auto polygon = port->getPolygon(j);
    
                if (polygon->numPoints > 0) {
                    double min_x = polygon->x[0], min_y = polygon->y[0];
                    double max_x = polygon->x[0], max_y = polygon->y[0];
                    
                    for (int k = 1; k < polygon->numPoints; k++) {
                        min_x = std::min(min_x, (double)polygon->x[k]);
                        min_y = std::min(min_y, (double)polygon->y[k]);
                        max_x = std::max(max_x, (double)polygon->x[k]);
                        max_y = std::max(max_y, (double)polygon->y[k]);
                    }
                    
                    cur_port->rects_.emplace_back(min_x, min_y, max_x, max_y);
                }
            }
        }
